...
export default function ArtistTrackItem({ name, id, avatarUrl, artist, ratingTrackOverall }) {
  ...
  return (
    <li className="artist__track-item">
        <Link to={`/track/${id}`}>
            <img src={`http://localhost:5000${avatarUrl}`} alt="" className="artist__track-img" />
        </Link>
                    ...
                    artist.map(e => (
                        <Link to={`/artist/${e._id}`} key={e._id}>
                            <div className="artist__track-artist-wrapper">
                                <img src={`http://localhost:5000${e.avatarUrl}`} alt={e.name} className="artist__track-artist-avatar" />
